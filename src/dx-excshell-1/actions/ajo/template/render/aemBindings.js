...
const { unifiedPromotionalContentSelection, prbPropertiesSelection } = require("./queries");
...
function isSufficientBindingValue(model, value) {
  ...
  if (model === "unifiedPromotionalContent") {
    // bodyCopy is now part of the selection set; but we still treat headline/image as "sufficient"
    if (!value.headlineText && !value.eyebrowText && !value.primaryImage && !value.bodyCopy) return false;
    return true;
  }
...
async function resolveAemBindingValues({ stitchedHtml, params }) {
  ...
  const results = await mapLimit(misses, concurrency, async ({ binding, model, skey }) => {
    ...
    const selectionSet = model === "unifiedPromotionalContent" ? unifiedPromotionalContentSelection : prbPropertiesSelection;
