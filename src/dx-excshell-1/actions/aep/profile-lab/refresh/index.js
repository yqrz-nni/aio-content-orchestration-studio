...
async function main(params) {
  ...
  try {
    ...
    const sourceTable = String(params.AEP_PROFILE_LAB_SOURCE_TABLE || params.AEP_PROFILE_LAB_TABLE || '').trim()
    const snapshotTable = String(params.AEP_PROFILE_LAB_SNAPSHOT_TABLE || '').trim()
    ...
    if (!sourceTable) return badRequest('Missing AEP_PROFILE_LAB_SOURCE_TABLE (or AEP_PROFILE_LAB_TABLE).')
    if (!snapshotTable) return badRequest('Missing AEP_PROFILE_LAB_SNAPSHOT_TABLE.')
    ...
    if (op === 'start') {
      const sql = client.buildSnapshotRefreshSql({
        sourceTable,
        snapshotTable,
        limit: toPositiveInt(params.limit, 2000)
      })

      const started = await client.startQuery({
        dbName,
        sql,
        queryName: String(params.AEP_PROFILE_LAB_REFRESH_QUERY_NAME || 'Profile Lab Snapshot Refresh').trim(),
        description: 'Rebuild shared Profile Lab snapshot table from source dataset.'
      })

      if (!started.queryId) return serverError('Snapshot refresh query submission did not return an id.')

      return ok({ op, queryId: started.queryId, state: started.state || 'SUBMITTED', ready: false })
    }

    if (!queryId) {
      return badRequest('Missing queryId for non-start operation.')
    }
